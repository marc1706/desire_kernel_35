...
static inline int test_bit_id(int nr, const volatile unsigned long *addr)
{
#ifdef LOW_ID_PIN
	return 0;
#else
	return 1;/*test_bit(nr, addr); */
#endif
}
...
static int is_host(void)
{
	...
	if (dev->pmic_id_notif_supp)
		...
		return 1;
#else
		return 0; /*(OTGSC_ID & readl(USB_OTGSC)) ? 0 : 1; */
#endif
	else
...
static int msm_otg_set_power(struct otg_transceiver *xceiv, unsigned mA)
{
	...
	USBH_DEBUG("Charging with %dmA current\n", charge);
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_WAIT_BCON:
		if ((test_bit_id(ID, &dev->inputs) &&
			...
			spin_unlock_irq(&dev->lock);
			msm_otg_start_timer(dev, TA_WAIT_VFALL, A_WAIT_VFALL);
		} else if (test_bit_id(ID, &dev->inputs) &&
				!test_bit(ID_A, &dev->inputs)) {
			USBH_DEBUG("id && !id_a\n");
			dev->otg.default_a = 0;
			/* Reset both phy and link */
